...
void PMS::generateReport(JsonArray &data, DynamicJsonBuffer &buffer, float pm1, float pm25, float pm10) {
...
bool PMS::report(JsonArray &data, DynamicJsonBuffer &buffer) {
  ...
  float pm1 = pm1sum / (samples - errors);
  float pm25 = pm25sum / (samples - errors);
  float pm10 = pm10sum / (samples - errors);

  generateReport(data, buffer, pm1, pm25, pm10);
